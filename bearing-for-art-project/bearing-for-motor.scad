...
MOTOR_SCREW_OFF_CENTER=8;
// Extra thickness of the frame to compensate for the motor pin build
MOTOR_PIN_BUILD=1.55; // measured: 1.5, but we leave some extra room
MOTOR_PIN_ROUND_PART=2.5;
PLANET_HOLE_D=3;
	...
	for(i=[1:m])rotate([0,0,i*360/m+phi])translate([pitchD/2*(ns+np)/nr,0,0])
		rotate([0,0,i*ns/m*360/np-phi*(ns+np)/np-phi])
            difference() {
                ...
                if (i == 1)
                    cylinder(r=PLANET_HOLE_D/2,h=T+1,center=true,$fn=100);
            }
}
